// neurodigineration — ask page (GUI 2: end-user neurodegen Q&A).
//
// Reads model state from the SAME localStorage key as the training GUI
// (`nd-train-v1`). That means every rating you make in /train.html
...
const ASK_HISTORY_KEY = 'nd-ask-history-v1';

const NEURODEGEN_SYSTEM_PROMPT = `You are neurodigineration, an AI research assistant focused on the molecular biology of neurodegenerative disease.
...
function loadConversation() {
  try {
    let raw = localStorage.getItem(ASK_HISTORY_KEY);
    if (!raw) {
      const legacy = localStorage.getItem('bioscope-ask-history-v1');
      if (legacy) {
        raw = legacy;
        try { localStorage.setItem(ASK_HISTORY_KEY, legacy); } catch { /* noop */ }
      }
    }
    if (!raw) return [];
...
function renderConversation() {
  ...
  for (const turn of state.conversation) {
    ...
    const turnEl = el('div', { class: `turn ${turn.role} ${turn.streaming ? 'streaming' : ''}` },
      el('div', { class: 'who' }, turn.role === 'user' ? 'You' : 'neurodigineration'),
      bubble,
      meta,
    );
...
async function ask() {
  ...
  try {
    ...
    console.error(err);
    const last = state.conversation[state.conversation.length - 1];
    last.content = `_neurodigineration: ${err.message || err}_`;
    last.streaming = false;
    renderConversation();
...
async function boot() {
  ...
  $('#export-conv').addEventListener('click', () => {
    ...
    a.href = url; a.download = 'neurodigineration-conversation.md'; a.click();
