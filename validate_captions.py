...
def validate_file(filepath):
    ...
    for i, line in enumerate(lines):
        if '<img' in line and 'src=' in line:
            ...
            if src_match:
                ...
                # Check context
                is_gallery = any('gallery-thumb' in lines[j] for j in range(max(0, i-3), i))
                has_figcaption = any('<figcaption' in lines[j] for j in range(i, min(len(lines), i+5)))
                has_div_caption = any('font-size:11px' in lines[j] and 'padding:4px' in lines[j] for j in range(i, min(len(lines), i+2)))
    ...
    for img in images_found:
        if img['is_gallery']:
            ...
            if not img['has_figcaption']:
                missing_captions.append(f"Standalone image '{img['filename']}' (line {img['line']}) missing figcaption")

    # Report
    print(f"  Total images: {len(images_found)}")
    print(f"  - Standalone with figcaption: {sum(1 for img in images_found if not img['is_gallery'] and img['has_figcaption'])}")
